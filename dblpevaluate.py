...
def evaluate(predictions, answers, labels_of_interest):
    # Calculate F1 scores
    f1_micro = f1_score(answers, predictions, average='micro')
    f1_macro = f1_score(answers, predictions, average='weighted')
    ...
    print("Confusion Matrix:")
    print(confusion_matrix(answers, predictions, labels=labels_of_interest, normalize='true'))

# Main function to execute the evaluation
def main():
    file_path = 'dataset/dblp/result/prediction.txt'  # Path to your prediction file
    labels_of_interest = ['machine_learning', 'theoretical_computer_science', 'computer_networking']  # Define your labels of interest
