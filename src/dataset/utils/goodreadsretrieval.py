...
def retrieval_via_pcst(graph, d_emb, textual_nodes, textual_edges, topk=3, topk_e=3, cost_e=0.5):
    c = 0.01
    ...
    verbosity_level = 0
    if topk > 0:
        n_prizes = torch.nn.CosineSimilarity(dim=-1)(d_emb, graph.x)
    ...
    if topk_e > 0:
        e_prizes = torch.nn.CosineSimilarity(dim=-1)(d_emb, graph.edge_attr)
